...
class Stats(object):
    ...
    def res_by_org(cls, limit=10):
        ...
        reses = connection.execute("select owner_org,format,count(*) from \
                resource inner join package on resource.package_id = package.id where package.state='active' and resource.state='active' and package.private='f' group by owner_org,format order by count desc;").fetchall();
        group_ids = []
        group_tab = {}
        group_spatial = {}
        group_other = {}
