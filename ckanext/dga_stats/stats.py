...
class Stats(object):
    ...
    def top_package_owners(cls, limit=10):
        ...
        if cache_enabled:
            key = 'top_package_owners'
            user_count = our_cache.get_value(key=key,
                                             createfunc=fetch_top_package_owners,
                                             expiretime=cache_default_timeout)
        else:
            user_count = fetch_top_package_owners()
        return user_count
    ...
    def summary_stats(cls):
        def fetch_summary_stats():
            ...
            return connection.execute("SELECT 'Total Organisations', count(*) from \"group\" where type = 'organization' and state = 'active' union \
    ...
    def users_by_organisation(cls):
        def fetch_users_by_organisation():
            connection = model.Session.connection()
        ...
        if cache_enabled:
            ...
            result = our_cache.get_value(key=key,
                                      createfunc=fetch_users_by_organisation,
                                      expiretime=cache_default_timeout)
        else:
            result = fetch_users_by_organisation()
    ...
    def recent_created_datasets(cls):
        ...
        if cache_enabled:
            ...
            r = our_cache.get_value(key=key,
                                      createfunc=fetch_recent_created_datasets,
                                      expiretime=cache_default_timeout)
        else:
            r = fetch_recent_created_datasets()
    ...
    def recent_updated_datasets(cls):
        ...
        if cache_enabled:
            ...
            r = fetch_recent_updated_datasets()
